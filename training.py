...
# Метод str.strip() возвращает копию строки s у которой удалены все пробелы стоящие в начале и конце строки.
...
sorted(lst) # если требуется сохранить исходный список - a = [1, 3, 3], b = sorted(a). sorted() возвращает список
...
def is_pangram(text):
    ...
    textLst = [i.lower() for i in set(text) if i.isalpha()]  # функция set() принимает строку и возвращает неповторяющиеся (уникальные) символы из строки, убирает дубли
    for i in alph:
        if i not in textLst:
            return False
    return True
